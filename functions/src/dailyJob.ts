...
async function runDaily(geminiKey: string) {
  ...
  if (geminiKey) {
    ...
    const recap = await geminiCall(geminiKey, GMODEL,
      ...
      `באור שאני רואה / זה באנקר! / נביא את הבוחטיות / את הילד שלי אני שם על זה. ` +
    ...
    if (todayFixtures.length) {
      preview = await geminiCall(geminiKey, GMODEL,
        ...
        `מבין משחקי היום: ${todayFixtures.join(', ')}. בחר את המשחק הכי מסקרן וכתוב משפט הייפ אחד קצר בעברית (עד 130 תווים) ` +
        `על המשחק עצמו. אם אתם בטוחים בתחזית, שלבו ביטוי אופייני שלכם כמו "זה באנקר!" או "את הילד שלי אני שם על זה". ` +
        `אל תמציא עובדות (מארחת, דירוג, פציעות) שאינך בטוח בהן. בלי מרכאות.`, { maxTokens: 100 })
    }
