...
async function runDaily(geminiKey: string) {
  ...
  if (geminiKey) {
    ...
    const recap = await geminiCall(geminiKey, GMODEL,
      ...
      `חובה לשלב באחת השורות, באופן טבעי, את הביטוי המדויק "באור שאני רואה". ` +
    ...
    if (todayFixtures.length) {
      preview = await geminiCall(geminiKey, GMODEL,
        ...
        `מבין משחקי היום: ${todayFixtures.join(', ')}. בחר את המשחק הכי מסקרן וכתוב משפט הייפ אחד קצר בעברית (עד 120 תווים) ` +
        `על המשחק עצמו. אל תמציא עובדות (מארחת, דירוג, פציעות) שאינך בטוח בהן. בלי מרכאות.`, { maxTokens: 90 })
    }
